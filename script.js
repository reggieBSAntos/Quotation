...
const init = () => {
  ...
  const callbackForm = (returnValue) => {
    ...
    Array.from(document.querySelectorAll(".form .contents")).forEach(
      (fieldset) => {
        // Get the service
        objA.answers[fieldset.dataset.service] = Array.from(
    ...
    Array.from(document.querySelectorAll(".content.closing input")).forEach(
      (input) => (objA.info[input.dataset.inputtype] = input.value)
    );
    ...
    objA.serviceList = JSON.parse(JSON.stringify(serviceList));
