...
class Calendar {
  ...
  show() {
    if (document.querySelector(".calendar") != null) {
      this._close(document.querySelector(".calendar"));
      return;
    }
    let date = new Date();
    const renderCalendar = () => {
      ...
      const innerHtml = days
        .map((day, dayIndex) => {
          ...
          const isWeekEnd =
            newDate.getDay() === 0 || newDate.getDay() === 6 ? " weekend" : "";

          let isHoliday =
            this.options.holiday != null
              ? this.options.holidays.findIndex((r) => {
                  return r === newDateString;
                }) !== -1
                ? " holiday"
                : ""
              : "";
          ...
          return `<div class="day"
                data-date="${day}">
                  <span class="${isToday}${isWeekEnd}${isCurrent}${isHoliday}">
                  ${newDate.getDate()}
                  </span>
                </div>`;
        })
    ...
    calendar.addEventListener("click", (e) => {
      ...
      if (el.closest(".day--present:not(.weekend)")) {
        date = new Date(parseFloat(el.closest(".day").dataset.date));
        this.options.onSelect(date);
        this._close(calendar);
      }
